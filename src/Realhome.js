	...
	delete(e, element) {
		...
		if (confirm === true) {
			fetch("http://localhost:5000/rdelete", {
				...
				body: JSON.stringify({
					userid: window.localStorage.getItem("usn"),
					filename: element,
				}),
			})
				.then((res) => res.json())
			...
			<div onLoad={this.file}>
				<div id="bar1">
					...
					<div id="hd1">
						...
						></img>
						<Link to="/editor">
							<button id="bt100">Create</button>
						</Link>
						<Link to="/myprofile">
							<button id="bt10">My profile</button>
						</Link>
						<button
							id="bt200"
							onClick={(e) => {
						...
						{fileitemarr.map((element) => {
							return (
								<div>
									<div
										className="divfile"
										onClick={(e) => this.click(e, element)}
									>
										{element}
									</div>
									<span className="deletec">
										<img
											id="delete"
